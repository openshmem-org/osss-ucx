...
#define ALLTOALL_HELPER_BARRIER_DEFINITION(_name, _peer, _cond)         \
    inline static void alltoall_helper_##_name##_barrier(void *dest, const void *source, size_t nelems, \
        ...
        for (i = 1; i < PE_size; i++) {                                 \
            ...
            source_ptr = ((uint8_t *) source) + peer_as * nelems;       \
                                                                        \
            shmem_putmem_nbi(dest_ptr, source_ptr, nelems, PE_start + peer_as * stride); \
        }                                                               \
                                                                        \
...
#define ALLTOALL_HELPER_COUNTER_DEFINITION(_name, _peer, _cond)         \
    inline static void alltoall_helper_##_name##_counter(void *dest, const void *source, size_t nelems, \
                                                         int PE_start, int logPE_stride, int PE_size, long *pSync) { \
        const int stride = 1 << logPE_stride;                           \
        const int me = shmem_my_pe();                                   \
                                                                        \
...
#define ALLTOALL_HELPER_SIGNAL_DEFINITION(_name, _peer, _cond)          \
    inline static void alltoall_helper_##_name##_signal(void *dest, const void *source, size_t nelems, int PE_start, \
